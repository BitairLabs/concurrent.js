...
import type { divide, isPrime, isWorker, reactiveAdd } from './sample_services/index.ts'
declare type Services = {
  divide: typeof divide
  isPrime: typeof isPrime
  isWorker: typeof isWorker
  reactiveAdd: typeof reactiveAdd
}
...
const services = await concurrent.import<Services>(SERVICES_SRC).load()
...
Deno.test('should run a reactive function', async () => {
  ...
  const listener = (onmessage: IChannel['onmessage'], postMessage: IChannel['postMessage']) => {
